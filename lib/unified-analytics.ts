...
export class UnifiedAnalytics {
  ...
  public static async getAnalyticsMetrics(
    options: { excludeFamilyUnits?: boolean } = {},
  ): Promise<UnifiedAnalyticsMetrics> {
    ...
    try {
      ...
      console.log(
        `[UNIFIED_ANALYTICS] Processing ${units.length} units from master CSV ${options.excludeFamilyUnits ? "(excluding family units)" : "(all units)"}`,
      );
      ...
      const occupiedUnits = units.filter(
        (unit) => !unit.isVacant || unit.isFamily,
      );
      ...
      console.log(
        `[UNIFIED_ANALYTICS] Vacancy definition applied: ${vacantUnits.length} vacant, ${occupiedUnits.length} occupied`,
      );
      ...
      const allNonStudentUnits = units.filter(
        (unit) => !isStudentUnit(unit.unit),
      );

      const occupiedStudentUnits = occupiedUnits.filter((unit) =>
        isStudentUnit(unit.unit),
      );
      const occupiedNonStudentUnits = occupiedUnits.filter(
        (unit) => !isStudentUnit(unit.unit),
      );

      const occupancyStudent =
        allStudentUnits.length > 0
          ? (occupiedStudentUnits.length / allStudentUnits.length) * 100
          : 0;
      const occupancyNonStudent =
        allNonStudentUnits.length > 0
          ? (occupiedNonStudentUnits.length / allNonStudentUnits.length) * 100
          : 0;
      ...
      const avgVacancyDays =
        await this.calculateAverageVacancyDays(vacantUnits);

      // Calculate move activity
      const { moveInsMTD, moveOutsMTD } = await this.calculateMoveActivity();

      // Calculate financial metrics
      const actualMRR = occupiedUnits.reduce(
        (sum, unit) => sum + unit.monthlyRent,
        0,
      );
      const vacancyLoss = vacantUnits.reduce(
        (sum, unit) => sum + unit.marketRent,
        0,
      );
      const marketPotential = actualMRR + vacancyLoss;
      const arpu =
        occupiedUnits.length > 0 ? actualMRR / occupiedUnits.length : 0;
      ...
      console.error(
        "[UNIFIED_ANALYTICS] ❌ Failed to calculate analytics:",
        error,
      );
      throw error;
    }
  ...
  private static async getUnitsFromMasterCSV(): Promise<UnitData[]> {
    try {
      ...
      if (!masterData || masterData.length === 0) {
        console.warn(
          "[UNIFIED_ANALYTICS] No master CSV data found in database",
        );
        return [];
      }
      ...
      const useSmartDedup =
        process.env.USE_SMART_VACANCY_DEDUPLICATION !== "false";
      console.log(
        `[UNIFIED_ANALYTICS] Vacancy mode: ${useSmartDedup ? "SMART row selection (v18.3.0)" : "LEGACY row-based"}`,
      );
      ...
      const excludedFromVacant: Array<{
        unit: string;
        statuses: string[];
      ...
      const units: UnitData[] = Array.from(unitGroups.entries()).map(
        ([unitCode, records]) => {
          const uniqueStatuses = [
            ...new Set(records.map((r) => r.tenantStatus || "")),
          ];
          const hasVacant = uniqueStatuses.some(
            (s) => s.toLowerCase() === "vacant",
          );
          const hasNonVacant = uniqueStatuses.some(
            (s) => s.toLowerCase() !== "vacant" && s !== "",
          );
          ...
          // Derive classification using centralized logic
      ...
      if (excludedFromVacant.length > 0 && useSmartDedup) {
        console.log(
          `[UNIFIED_ANALYTICS] 📋 Excluded ${excludedFromVacant.length} units from vacant count (have Future/Notice tenants):`,
        );
        excludedFromVacant.forEach(({ unit, statuses, selectedStatus }) => {
          console.log(
            `  - ${unit}: [${statuses.join(", ")}] → Using: ${selectedStatus}`,
          );
        });
      }

      console.log(
        `[UNIFIED_ANALYTICS] Loaded ${masterData.length} CSV rows → ${units.length} unique units`,
      );
      console.log(`[UNIFIED_ANALYTICS] Vacancy breakdown:`, {
        ...
        vacantStatuses: [
          ...new Set(
            units.filter((u) => u.isVacant).map((u) => u.tenantStatus),
          ),
        ],
        occupiedStatuses: [
          ...new Set(
        ...
        studentUnits: units.filter((u) =>
          u.tenantType?.toLowerCase().includes("student"),
        ).length,
      });
      ...
      console.error(
        "[UNIFIED_ANALYTICS] ❌ Failed to get master CSV data:",
        error,
      );
      throw error;
    }
  ...
  private static calculateFamilyMetrics(
    units: UnitData[],
  ): UnifiedAnalyticsMetrics["familyUnits"] {
    ...
    const familyActualMRR = familyUnits.reduce(
      (sum, unit) => sum + unit.monthlyRent,
      0,
    );
    const familyMarketPotential = familyUnits.reduce(
      (sum, unit) => sum + unit.marketRent,
      0,
    );
   ...
   */
  private static async calculateAverageVacancyDays(
    ...
    try {
      ...
      const totalDays = vacantUnitsWithDays.reduce(
        (sum, unit) => sum + (unit.daysVacant || 0),
        0,
      );
      return Math.round(totalDays / vacantUnitsWithDays.length);
    } catch (error) {
      console.warn(
        "[UNIFIED_ANALYTICS] Failed to calculate average vacancy days:",
        error,
      );
      return 0;
    }
   ...
   */
  private static async calculateMoveActivity(): Promise<{
    moveInsMTD: number;
    moveOutsMTD: number;
  }> {
    try {
      // Try to get move activity from existing analytics functions if available
      const { getMoveInsMTD, getMoveOutsMTD } = await import(
        "./occupancy-analytics"
      );
      ...
      console.warn(
        "[UNIFIED_ANALYTICS] Failed to calculate move activity:",
        error,
      );
  ...
  private static async getLatestSnapshotDate(): Promise<string> {
    try {
      ...
      if (latestFromTenantData._max.snapshotDate) {
        ...
        return latestFromTenantData._max.snapshotDate
          .toISOString()
          .split("T")[0];
      }
      ...
      if (latestFromCsvData._max.updatedAt) {
        const easternDate = EasternTimeManager.toEasternDate(
          latestFromCsvData._max.updatedAt,
        );
        if (easternDate) {
          return easternDate;
        }
      ...
      console.warn(
        "[UNIFIED_ANALYTICS] No snapshot date found in data, using today's date",
      );
      return EasternTimeManager.getCurrentEasternDate();
    } catch (error) {
      console.error(
        "[UNIFIED_ANALYTICS] Error getting latest snapshot date:",
        error,
      );
      return EasternTimeManager.getCurrentEasternDate();
    }
